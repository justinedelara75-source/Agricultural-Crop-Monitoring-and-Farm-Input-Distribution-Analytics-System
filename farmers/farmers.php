...
$stmt    = $pdo->query("SELECT * FROM farmers ORDER BY last_name ASC");
...
.fm-wrap { font-family:'DM Sans',sans-serif; padding:24px 28px 60px; }
.fm-wrap * { box-sizing:border-box; }

/* Header */
.fm-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; gap:12px; }
.fm-header-left { display:flex; align-items:center; gap:10px; }
.fm-header-left h2 { margin:0; font-size:1.35rem; font-weight:700; color:var(--gr900); }
...
.farmer-mid   { font-size:.78rem; color:var(--gr400); display:block; margin-top:1px; }

.brgy-badge {
    display:inline-flex; padding:3px 10px; border-radius:20px;
    font-size:.72rem; font-weight:700; background:var(--b100); color:var(--b700);
}
...
<div class="fm-wrap">
    ...
    <div class="fm-header">
        ...
        <div class="fm-actions">
            <a href="add_farmer.php" class="btn-act btn-green">+ Add Farmer</a>
    ...
    <div class="summary-strip">
        <span class="summary-badge">👨‍🌾 All Farmers</span>
        <span class="summary-count"><?= count($farmers) ?> record(s) found</span>
    </div>
            ...
            <table>
                ...
                <tbody>
                    <?php if (empty($farmers)): ?>
                        <tr class="empty-row">
                            <td colspan="7">No farmers found. Click <strong>+ Add Farmer</strong> to get started.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($farmers as $i => $f): ?>
                            <tr class="clickable-row" data-id="<?= $f['id'] ?>">
                                ...
                                <td>
                                    ...
                                    <?php if (!empty($f['middle_name'])): ?>
                                        <span class="farmer-mid"><?= htmlspecialchars($f['middle_name']) ?></span>
                                    <?php endif; ?>
                                </td>
                                <td class="center">
                                    <span class="brgy-badge"><?= htmlspecialchars($f['barangay']) ?></span>
                                </td>
                                <td class="center">
                                    <?php
                                    $bd = $f['birthdate'] ?? '';
                                ...
                                <td class="num"><?= number_format($f['farm_size'], 2) ?> ha</td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
...
document.getElementById('editBtn').onclick = () => {
    if (selectedId) location.href = 'edit_farmers.php?id=' + selectedId;
};

document.getElementById('deleteBtn').onclick = () => {
    if (selectedId && confirm('Delete this farmer?')) {
        location.href = 'delete_farmer.php?id=' + selectedId;
    }
};
</script>
