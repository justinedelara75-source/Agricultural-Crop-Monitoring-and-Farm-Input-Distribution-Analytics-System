...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($last == "" || $first == "" || $barangay == "" || $farm_size == "") {
        ...
        $check = $pdo->prepare("SELECT COUNT(*) FROM farmers WHERE last_name=? AND first_name=? AND middle_name=?");
        $check->execute([$last, $first, $middle]);
        if ($check->fetchColumn() > 0) {
            $error = "Farmer already exists!";
        } else {
            $stmt = $pdo->prepare("
                INSERT INTO farmers (last_name, first_name, middle_name, barangay, contact_number, email, farm_size, birthdate)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$last, $first, $middle, $barangay, $contact, $email, $farm_size, $birthdate ?: null]);
...
.af-wrap { font-family:'DM Sans',sans-serif; padding:24px 28px 60px; }
.af-wrap * { box-sizing:border-box; }

.af-header { display:flex; align-items:center; gap:10px; margin-bottom:24px; }
.af-header h2 { margin:0; font-size:1.35rem; font-weight:700; color:var(--gr900); }
...
.f-input.invalid:focus { box-shadow:0 0 0 3px rgba(239,68,68,.12); }
    ...
    <div class="form-card">
        <form method="POST" id="addFarmerForm">
            ...
            <div class="action-bar">
                <button type="submit" class="btn-act btn-green">✓ Save Farmer</button>
