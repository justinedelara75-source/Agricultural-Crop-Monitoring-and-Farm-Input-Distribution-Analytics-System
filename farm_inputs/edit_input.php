...
// JOIN farmers to get name, barangay, birthdate, farm_size
$stmt = $pdo->prepare("
    SELECT 
        fi.id,
        fi.farmer_id,
...
.readonly-label {
    ...
    background:var(--gr100); color:var(--gr500); margin-left:6px;
}
...
.btn-act {
    padding:10px 22px; border-radius:var(--rad-s); font-size:.875rem;
    font-weight:600; font-family:'DM Sans',sans-serif; cursor:pointer;
    border:none; transition:var(--tr); text-decoration:none; display:inline-flex; align-items:center; gap:6px;
}
        ...
        <form method="POST">
            ...
            <div class="section-row">
                <div class="section-label">
                    ...
                    <span class="s-desc">Farmer details from the registry</span>
                </div>
                <div class="section-fields">
                    <div class="field field-span3">
                        <label>Farmer Name <span class="readonly-label">🔒 Read-only</span></label>
                        <input type="text" class="f-input" readonly
                            value="<?= htmlspecialchars($data['last_name'] . ', ' . $data['first_name'] . ' ' . $data['middle_name']) ?>">
                    </div>

                    <div class="field">
                        <label>Barangay <span class="readonly-label">🔒 Read-only</span></label>
                        <input type="text" class="f-input" readonly
                            value="<?= htmlspecialchars($data['barangay']) ?>">
                    </div>
                    <div class="field">
                        <label>Birthdate <span class="readonly-label">🔒 Read-only</span></label>
                        <input type="text" class="f-input" readonly
                            value="<?php
                                $bd = $data['birthdate'] ?? '';
                                echo (!empty($bd) && $bd !== '0000-00-00')
                                    ? date('M d, Y', strtotime($bd))
                                    : '-';
                            ?>">
                    </div>
                    <div class="field">
                        <label>Farm Size <span class="readonly-label">🔒 Read-only</span></label>
                        <input type="text" class="f-input" readonly
                            value="<?= number_format($data['farm_size'], 2) ?> ha">
                    </div>
                </div>
            </div>
                ...
                <div class="section-fields">
                    <div class="field field-span2">
                        ...
                        <input class="f-input" name="rsbsa_no"
                            value="<?= htmlspecialchars($data['rsbsa_no']) ?>"
            ...
            <div class="section-row">
                ...
                <div class="section-fields" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0 20px;padding:24px 28px;align-content:start;">
                    <div class="field">
                        <label>Senior Citizen</label>
                        <select name="senior_citizen" class="f-select">
                            <option value="N" <?= $data['senior_citizen'] == 'N' ? 'selected' : '' ?>>No</option>
                            <option value="Y" <?= $data['senior_citizen'] == 'Y' ? 'selected' : '' ?>>Yes</option>
                        </select>
                    </div>
                    <div class="field">
                        <label>Indigenous People (IP)</label>
                        <select name="ip" class="f-select">
                            <option value="N" <?= $data['ip'] == 'N' ? 'selected' : '' ?>>No</option>
                            <option value="Y" <?= $data['ip'] == 'Y' ? 'selected' : '' ?>>Yes</option>
                        </select>
                    </div>
                    <div class="field">
                        <label>Person w/ Disability (PWD)</label>
                        <select name="pwd" class="f-select">
                            <option value="N" <?= $data['pwd'] == 'N' ? 'selected' : '' ?>>No</option>
                            <option value="Y" <?= $data['pwd'] == 'Y' ? 'selected' : '' ?>>Yes</option>
                        </select>
                    </div>
                </div>
            </div>
                ...
                <div class="section-fields">
                    <div class="field">
                        ...
                        <input type="date" class="f-input" name="est_date_application"
                            value="<?php
                                $edate = $data['est_date_application'] ?? '';
                                echo (!empty($edate) && $edate !== '0000-00-00') ? $edate : '';
                            ?>">
                    </div>
                </div>
            </div>
